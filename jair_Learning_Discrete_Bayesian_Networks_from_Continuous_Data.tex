...
Finding optimal discretization policies according to equation~\ref{eq:opt_prob} via dynamic programming requires long computation times when the dataset has more than \num{100} variables.
The runtime for the MDL approach is even more severe.
Approximation approaches are reasonable alternatives for datasets with more variables, including a greedy heuristic~\cite{Friedman_1996}.
Approximations can also be applied during dynamic programming when solving equation~\ref{eq:opt_prob}.
