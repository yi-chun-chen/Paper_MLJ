...
The fourth principle is that the distributions for each interval are independent, so the factors can be multiplied together. Note that the combinatorics of factors ${{\gamma_i + J_\text{P} - 1}\choose{J_\text{P} - 1}}$ and $\frac{
	{\gamma_i}!}{{n^{(\text{P})}_{i,1}}! \cdots \; {n^{(\text{P})}_{i,J_\text{P}}}!}$ are explained in MODL \citep{Boulle_2006}.
...
Approximation approaches are reasonable alternatives for datasets with more variables, including a greedy heuristic~\citep{Friedman_1996}.
