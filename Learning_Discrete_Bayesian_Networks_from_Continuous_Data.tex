...
To support the argument, a synthetic dataset with $800$ samples and under the network structure in Figure~\ref{fig:exp_discuss} is tested. Data samples are generated by the distribution described in Table~\ref{table:distri_exp}: both $P_1$ and $P_2$ take two discrete values with equal probability. $\cX$ samples numeric numbers given the value of $P_1$ and $P_2$. Note that the symbol $\sim U[a,b]$ in Table~\ref{table:distri_exp} represents an uniform distribution from value $a$ to $b$. The optimal discretization policy on $\cX$ should have six edges $\curly{2.0,3.0,4.0,4.5,5.0,6.0}$, since $\cX$ takes numeric values from $1.0$ to $7.5$ and there are seven intervals corresponding to different joint distribution of $P_1$ and $P_2$.

\begin{table}[ht]
	\centering
	\caption{The distribution of variables in Figure~\ref{fig:exp_discuss} for the proposed experiment. $P_1$ and $P_2$ are discrete variable. $\cX$ is continuous valued.}
...
Figure~\ref{fig:synthetic} shows how the likelihood of tested data and number of discretization edges on $\cX$ vary with respect to the size of trained data. Trained data are drawn randomly from the synthetic dataset. The remaining ones are tested data. If the size of the trained data is small, it is hard to recognize the underlying pattern of data samples and discretize the continuous variable $\cX$ accordingly. In this case, both likelihood of the trained data and number of discretization edges is small. However, since the Bayesian method is more sensitive to the data distribution, it has better performance of the MDL method in terms of likelihood and lays down more discretization edges. As the size of the trained data grows, both methods improve their accuracy and approach to optimal number of discretization edges. The Bayesian method finds the six discretization edges faster than the MDL method. Finally, as the size of the trained data is large enough to capture the data distribution, both methods can locate the six discretization edges and lead to same accuracy to the tested data. The curves in Figure~\ref{fig:synthetic} demonstrate that the Bayesian method is indeed more sensitive to the dataset than the MDL method. Note that all the results in Figure~\ref{fig:synthetic} are averaged over one hundred times.
...
\begin{figure}
	...
	\caption{The average log-likelihood of the trained data and the average number of discretization edges for $\cX$ as the number of trained data is varied. Trained data are drawn randomly from the synthetic dataset. The remaining ones are tested data. Results are averaged over one hundred times.}
	\label{fig:synthetic}
\end{figure}
