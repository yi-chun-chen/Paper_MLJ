...
A common discretization method for Bayesian networks is to split continuous variables into uniform-width intervals or to use field-specific expertise \citep{scott1979optimal, freedman1981histogram, knuth2013optimal, scargle2013studies}.
...
All experiments set $L$ to the largest number of intervals among discrete variables in $\cX$'s Markov blanket, which encourages Bayesian networks with consistent interval counts.
In some cases it may be desirable to use higher values for $L$, such as when discrete variables are binary and the continuous variable requires higher fidelity.
...
\tim{Did we also normalize each variable to be zero mean and unit covariance?}
...
Trained data are drawn randomly from the synthetic dataset.
\todo{The remaining ones are tested data.}
...
In this case, both the likelihood of the training data and number of discretization edges is small.
...
\begin{figure}
	...
	\caption{The average log-likelihood of the trained data and the average number of discretization edges for $\cX$ as the number of training samples is varied. Training data are drawn randomly from the synthetic distribution. \todo{The remaining ones are tested data.} \tim{$\leftarrow$ What does this mean?} Results are averaged over one hundred trials.
  \tim{Why do we plot training set likelihood and not test set likelihood?}}
	\label{fig:synthetic}
\end{figure}
...
This section provides a counterexample of marginal discretization methods. Different from more principled methods, as such the MDL discretization or the proposed Bayesian approach, the marginal discretization methods ignore the dependency of data samples between different variables. Therefore, it might lead to worse discretization outcomes. Here is an example: consider a synthetic dataset with two variable $A$ and $B$, with $500$ data samples generated by the Bayesian network $A \rightarrow B$ and the probability distribution shown in Table \ref{table:marginal_counter}. Following the experiment procedure in Section \ref{sec:experiments}, both the MDL method and the proposed Bayesian approach find a discretization edge of $B$ around $1.0$ and have log likelihood $-0.023$. On the other hand, while discretizing variable $B$ based on its marginal distribution of samples, the Bayesian block and other marginal discretization can not locate a discretization edge and result in log likelihood $-0.69$. This example shows that the discretization method without considering the dependency between variables might lead to fatal outcomes.

\begin{table}[ht]
	\centering
	\caption{Probability distributions for the variables in the network $A \rightarrow B$, in which $A$ is a discrete variable and $B$ is a continuous one
	}
