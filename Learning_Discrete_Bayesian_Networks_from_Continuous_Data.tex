...
The $\lambda_{1:k}$ representation is the number of instances before every discretization edge whereas the $\gamma_{1:k}$ representation is the number of instances within each discretization interval. Table \ref{table:data_demo} and \ref{table:data_demo_2} make an example of variables defined in this subsection.
...
\begin{table}[ht]
  \centering
  \caption{
    An example dataset that demonstrates variables defined in Section \ref{subsec:disc_policy} to discretize continuous variable $\cX$.
  }
  \input{Table0.tex}
  ...
  \centering
  \caption{
  	Illustrate variables defined in Section \ref{subsec:disc_policy} along with the dataset in Table \ref{table:data_demo}. Note that $\Lambda^{\text{ex}}_{\cX}$ is an example of a valid discretization policy. $\anglebrackets{}_{e}$, $\anglebrackets{}_{\lambda}$, and $\anglebrackets{}_{\gamma}$ are the three equivalent representations of discretization policies.
 }
  	\begin{tabular}{cc}
  		\toprule
  		Variable &  Value \\
...
Six additional datasets \citep{Lichman_2013} are tested to compare the proposed Bayesian approach and MDL discretization method. The results are summarized in Table \ref{table:more_data_set}. For each dataset, number of samples $n$, number of variables $m$, the fixed network to execute discretization, and resulted log-likelihood by the two methods are shown in the same row. Note that the fixed network structures are obtained by the same principle as in previous three datasets. The network representation in Table \ref{table:more_data_set} records the parent variables of each variable. For example, in the first row, the representation $\curly{\emptyset,A,BE,AB,B,AH,BDG,B}$ indicates that $\Pa_A = \emptyset$, $\Pa_B = A$, $\Pa_c = \curly{B,E}$, etc. Again, the results show that the proposed Bayesian discretization method outperforms the MDL method in terms of likelihood.

\begin{table}[ht]
	\centering
	\caption{
		The discretization results on several datasets.
	    Each row corresponds to a dataset. Names of datasets, numbers of samples, numbers of variables, network structures for discretization, log-likelihood obtained by the proposed Bayesian method and MDL method are shown.
	}
	\input{Table3_2.tex}
...
To further demonstrate the above discussion, the discretization results on Iris dataset with respect to different cropped data sizes are analyzed. As shown in Appendix, the results of the two methods coincide on Iris dataset under naive Bayes structure. However, if the dataset is cropped and the distribution pattern of data samples becomes less obvious, the two discretization methods would have different outcomes. 

Figure \ref{fig:iris_four} shows the relation between number of samples in cropped datasets and number of resulted discretization edges on each continuous variable. Samples are drawn randomly from the dataset and the result of number of edges are averaged over one thousand times. On variable $C$ and $D$, since data distribution is easier to discriminate, both methods have already located two discretization edges even if there were only thirty samples (one fifth of the entire dataset). On variable $A$ and $B$, more data samples are required to locate the discretization edges. Under this case, the proposed Bayesian method is more sensitive than the MDL method and able to lay down more discretization edges even if the data distribution is still vague for the MDL method.
...
\begin{figure}
	...
	\begin{subfigure}{0.45\textwidth}
		...
		%\label{fig:iris_d}
	\end{subfigure}
	\caption{Number of resulted discretization edges on each continuous variable with respect to number of data samples}
	\label{fig:iris_four}
\end{figure}
