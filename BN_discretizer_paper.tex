...
The first is to model the conditional probability density of each continuous variable using specific families of parametric distributions, and then to redesign Bayesian network learning and inference algorithms based on their parameters.
...
Under the suggested prior, which includes the uniform prior probabilities in MODL as a special case, the runtime can be reduced to $\bigo\paren{r \cdot n^2}$ without sacrificing optimality.
