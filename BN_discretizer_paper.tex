...
Today it is common practice to discretize all continuous variables before learning a Bayesian network structure to avoid having to consider variable interactions, as the interations and dependencies between variables in Bayesian networks introduce complexity.
...
Equation \ref{eqn:MDL} can be minimized using dynamic programming with runtime $\bigo \paren{ n^3 + \paren{ n_c \cdot {L}^{n_s}  + {L}^{n_p}} \cdot n^2 }$.
...
To quantatively discuss the sensitivity of each method to the number and position of discretization intervals, consider the Bayesian network in Figure~\ref{fig:exp_discuss}, where $\cX$ is continuous and $P_1$ and $P_2$ are discrete.
...
The sensitivity of each method to the discretization edges are captured in the edge position terms, as the penalty terms are similar across methods.
...
The MDL term uses emperical probability distributions, based off of ratios of counts, whereas the Bayesian method uses factorial terms, and thus varies less.
