...
Bayesian networks \citep{Pearl_1988, PGM_2009}, which first first emerged from artificial intelligence research, are an increasingly popular method for modeling uncertainty and causality with applications ranging from decision-making systems \citep{kochenderfer2012next} to medical diagnoses \citep{Lustgarten_2011}.
...
In most cases, one assumes that the random variables in a Bayesian network are discrete, since many Bayesian network learning algorithms are unable to efficiently handle continuous variables.
...
The first is to model the conditional probability density of each continuous variable using specific families of parametric distributions, and then to redesign Bayesian network learning and inference algorithms based on the distribution parameters.
...
Today it is common practice to discretize all continuous variables before learning a Bayesian network structure, which does not consider variable interactions, as interations and dependencies between variables in Bayesian networks are more complicated; discretization of a variable in a network requires considering the impact on the variable's Markov blanket.
...
The optimal discretization policy of a single continuous variable under MDL is found using dynamic programming, and has runtime $\bigo\paren{n^3 + q \cdot n^2}$, where $n$ is the number of data samples and $q$ is a constant with exponential dependency on the cardinalities of the variables in the Bayesian network.
...
MDL discretization requires that the network structure be known in advance, but can be incorporated into the structure learning process which would otherwise have required pre-discretized variables.
This approach alternates between traditional discrete structure learning and optimal discretization. \tim{Is this something this paper proposes or was this done with MDL before?}
Starting with some preliminary discretization policy, one first applys a structure learning algorithm to identify the locally optimal graph structure.
One then refines the discretization policy based on the learned network.
The cycle is repeated until convergence.

Results in this work suggest that the MDL method suffers from low sensitivity to discretization edges location and returns low numbers of discretization intervals on continuous variables.
\tim{We need another sentence to make this a \P.}
...
\cite{Lustgarten_2011} suggest several formulations for the prior term $P(\text{Model})$, and the asymptotical equivalence between MDL and MODL on the single-variable, single-class problem was examined in \citep{VL_2000}.
...
The space of acyclic graphs is superexponential in the number of nodes, and it is common to rely on heuristic search strategies \citep{PGM_2009}.
...
The two factors on the right hand side come from the second principle: all distributions of values of $\Pa_{\cX}$ in a given interval are equiprobable.
...
Note that because the objective function is cumulative over intervals; if a partition of $X$ into $k$ intervals by $\Lambda = \curly{\gamma_1 , \gamma_2  , \ldots , \gamma_k}$ is an optimal discretization policy, then any subinterval is optimal for the corresponding subproblem.
...
In this paper, prediscretization assigns $k$ equal-width intervals to each continuous variables, where $k$ is largest value of the number of intervals of initially discrete variables in the network.
...
\todo{move relevant notation here}
...
This section describes how the proposed discretization method can be combined with the K2 structure learning algorithm \citep{K2} to simultaneously perform Bayesian structure learning and discretization of continuous variables.
