...
The section describes experiments designed to test the soundness of the proposed method. The experiments were run on four datasets from the Irvine repository \citep{Lichman:2013}. For each dataset, the experiment consists of two parts. In the first part, the Bayesian network structure is known in advance, which is obtained by equal-width prediscretization and the K2 algorithm. In the second part, the Bayesian netowrk structure is not provided in advance, and it is learned along with the discretization process.

For the case that network structure is given, a comparison between the proposed method and the MDL principle discretization is made. 10-fold cross validation of log-likelihood on data set $D_{\text{all}}$ is calculated as follows: firstly, learn discretization policies of each continuous variables from train data $D_{\text{train}}$. Secondly, obtain conditional probability tables $\phi_{x^* \mid \Pi^*_x}$ from the discretized train data $D^*_{\text{train}}$. Thirdly, For a test data instance $(x_1,x_2, \ldots, x_n)$ from $D_{\text{test}}$, the likelihood is
\begin{equation}
P(x_1, \ldots, x_n) = \prod_{i=1}^{n} \phi_{x^*_i \mid \Pi^*_{x_i}} \cdot \left( {1\over{M'_{X_i}[j_i+1] - M'_{X_i}[j_i]}} \right)^{\boldsymbol{1}_{ \left\lbrace X_i \text{ is continuous}  \right\rbrace }}
\end{equation}
, where $M'_{X_i}$ is the augmented discretization policy on continuous variable $X_i$ such that $M'_{X_i}[1] = min(D_{\text{all},X})$, $M'_{X_i}[j] = M_{X_i}[j-1]$ for $j=2,...,k_i$, and $M'_{X_i}[k_i+1] = max(D_{\text{all},X})$, $k$ is the number of intervals of continuous variable $X_i$ after discretization. Furthermore, $M'_{X_i}[j_i] \leq x_i < M'_{X_i}[j_i +1]$. This can be interpreted as follows: once a discrete-valued Bayesian network is learned from train data, test data are sampled from the learned network. If data is continuous, then it is uniformly sampled from the corresponding discretization interval.


\subsection{Dataset 1: Auto MPG} 
\label{subsec:autompg}

In this subsection, two experiments are conducted on the Auto MPG dataset from the Irvine repository \citep{Lichman:2013}. This dataset has 392 instances (delete the 6 instance with missing data) and eight variables. Three variables are discrete: $B$, $G$, and $H$ (in the following experiment, variables are named and ordered according to the dataset information webpage). They have 5, 13, and 3 cardinality numbers, respectively. In the first experiment, the network structure is given. In the second experiment, the network structure is not provided.

\subsubsection{Experiment 1 on Auto MPG dataset}
\label{subsubsec:autompg_exp1}
The proposed discretization method and the MDL discretization method are tested on the Bayesian network structure in Fig. \ref{Auto_graph_1}, which is obtained by running K2 algorithm 1000 times on prediscretized data, each time with a random restart. The prediscretization is done by equal-width discretization on continuous variables by 5 intervals (median of cardinalities of discrete variables). Shaded nodes in Fig. \ref{Auto_graph_1} means that they are discrete-valued variables.
...
\centering
    ...
    \end{tabular}
  \caption{The Bayesian network obatined by K2 algorithm and prediscretization on Auto MPG dataset}
  \label{Auto_graph_1}
\end{figure}
...
\caption{Discretization result of Auto MPG dataset based on the graph in Fig. \ref{Auto_graph_1}}
...
\subsubsection{Experiment 2 on Auto MPG dataset}
\label{subsubsec:autompg_exp2}
