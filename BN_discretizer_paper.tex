...
An integer representation of $\Lambda_{\cX}$ can thus be written $\Lambda_{\cX} = \curly{\lambda_1 < \lambda_2  < \ldots < \lambda_k}$, where $\lambda_1 = s_1$ and $\lambda_i \in s_{1:m}$.
...
Four principles for the optimal discretization policy enable the formulation of $P(\Lambda)$ and $P(D_{-\cX} \mid \Lambda)$, where the probability of the data associated with the target variable $P(D_{\cX})$ is already captured in $P(\Lambda)$.
...
The Bayesian network graph structure causes the likelihood term $P(D \mid \Lambda)$ to factorizes according to $\cX$'s Markov blanket:
...
\begin{figure}[ht]
  \centering
  \input{graph1}
  \caption{Factorization of $P(D \mid \Lambda)$}
...
\begin{equation}
  ...
  P\paren{D_{\Pa_{\cX}} \mid \Lambda} = \prod_{i=1}^k
    \frac{1}{{{n_i + J_P - 1}\choose{J_P - 1}}}
    \frac{1}{
      \frac{
        {n_i}!
      }{
...
The log-inverse of $P(\Lambda) \cdot P(D|\Lambda)$ is minimized for computational convenience:
