...
The upper bound on the number of parents is common practice, and is used to prevent computing conditional distribtions with excessively large parameter sets.
...
\begin{equation}
P(D^* \mid \bm{\alpha}, \bm{\beta}, G) = \prod_{i=1}^N \prod_{j=1}^{q_i} \prod_{k=1}^{r_i}
\paren{
  \frac{
...
\noindent
where $\bm{\alpha}$ and $\bm{\beta}$ were previously defined for the Bayesian score (Equation~\ref{eq:BayesianScore}), and $\bm{\beta}^*$ is the set of observed counts in $D^*$.
The experiments below used a uniform Dirichlet prior of $\alpha_{ijk} = 1$ for all $i$, $j$, and $k$.
