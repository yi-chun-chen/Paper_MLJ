...
\yichun{A common discretization method for Bayesian networks is to split continuous variables by uniform width or by expertises from application fields. A more principled method is the minimum description length (MDL) principle discretization \citep{Friedman_1996}.}
...
\yichun{Equation \ref{eqn:MDL} can be minimized exactly using dynamic programming. For a given Bayesian network, the first three terms only depends on the number of discretization intervals $\lvert X \rvert$ and the fourth term is cumulative on the intervals. Hence, if $\Lambda = \anglebrackets{\gamma_1, \gamma_2, \ldots, \gamma_k}$ is a MDL optimal discretization policy with $k$ intervals, then  $\Lambda' = \anglebrackets{\gamma_1, \gamma_2, \ldots, \gamma_{k-1}}$ is a MDL optimal discretization policy of the corresponding subproblems with $k-1$ intervals.}
...
\begin{figure}[H]
  ...
  \caption{
    ...
    \yichun{The variable $H$ possesses less frequenly repeated values, thus producing less number of discretization intervals than $E$.}
  }
...
All software is publically available to support discretization research, and \yichun{can be found at \url{github.com/sisl/LearnDiscreteBayesNets.jl}}.
