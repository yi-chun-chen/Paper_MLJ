...
\begin{abstract}
Many applications of Bayesian networks assume all random variables are discrete. However, it is imperative to deal with continuous variables in the real-world data, such as positions and velocities of autonomous cars,  In this paper, a new method has been proposed to discretize continuous variables in Bayesian networks by learning from data.

 In addition, the new method can be executed along with edge-adding steps in the K2 structure learning algorithm, and thus learn a discrete-valued Bayesian network.
...
%     $\bm{S}_i$ & & set of spouses associated with $i$th child of specific var \\
...
In many application, it is assume that all random variables in Bayesian networks are discrete, since many Bayesian network algorithms are unable to deal with continuous variables efficiently. In addition, many of the commonly used Bayesian network software packages, such as \todo{find packages}, assume discrete variables. However, various applications require the use of continuous variables, such as position and velocity in dynamic systems \citep{kochenderfer2010airspace}.
...
Let the samples of $\cX$ in a given dataset $D$ be sorted in ascending order, ${\cx_{1:n} = \curly{\cx_1 \leq \cx_2 \leq \ldots \leq \cx_n}}$, and let the unique values be ${\cu_{1:m} = \curly{\cu_1 \leq \cu_2 \leq \ldots \leq \cu_m}}$.
...
Thus, each edge $e_i = \paren{\cu_{s_j} + \cu_{s_{j+1}}}/2$ for some $s_j$.
...
The four principles, motivated by MODL \todo{and Lustgarden}, are:
...
\begin{equation}
  ...
  \curly{
    \brock{
      ...
      L \cdot \frac{\cx_{s_{i}} - \cx_{s_{i-1} + 1}}{\cx_n - \cx_1}}
    }\text{.}
\end{equation}
...
Note that $\lambda_i = \sum_j n^{(\text{p})}_{i,j}$.
It follows that
...
\begin{equation}
  ...
  P\paren{D_{\Pa_{\cX}} \mid \Lambda} = \overbrace{\prod_{i=1}^k}^{\text{Principle 4}}
    \overbrace{\rule{0pt}{2em}
      \frac{1}{{{\lambda_i + J_P - 1}\choose{J_P - 1}}}
      \frac{1}{
        \frac{
          {\lambda_i}!
        }{
          {n^{(\text{p})}_{i,1}}! \; {n^{(\text{p})}_{i,2}}! \; \cdots \; {n^{(\text{p})}_{i,J_P}}!
        }
      }
    }^{\text{Principle 2}}\text{.}
\end{equation}
...
\subsubsection{Evaluation of $P(D_{C_i} \mid \Lambda, D_{\bm{S}_i})$}
...
Note that ${\lambda_i = \sum_l n^{(j)}_{i,l}}$.
It follows that
...
\begin{equation}
  ...
  P(D_{C_j} \mid \Lambda) = \prod_{i=1}^{k}
  {
    {1}\over{
      {\lambda_i + J_{C_j} - 1}\choose{J_{C_j}-1}}
    }
    {{1}\over{
      {{\lambda_i}!} \over {
...
Let ${D'_X} = \brock{x'_1,x'_2,\ldots,x'_\Lambda}$ be the set of unique values of $D_X$ sorted in ascending order, where $\Lambda$ is the number unique values in $D_X$.
Recall that $D_X$ is also sorted.
Let $b = \brock{b_0,b_1,b_2,\ldots,b_\Lambda}$ be an increasing sequence of integers marking the repeated value intervals in $D_X$ such that $b_0 = 0$ and $x_{b_{i-1} + 1} = x_{b_{i-1} + 2} = \ldots = x_{b_i} = x'_i$.
...
\State $b \leftarrow$ the increasing sequence of $\Lambda$ integers as previously defined
...
\begin{algorithm}
  ...
  \Function{Discretize}{$D''$, $G$, $C$, $u_{cycle}$}
    ...
    \While {$\Lambda$ is not converged  \textbf{and} $cycle \leq u_{cycle}$}
...
where $I(A,B)$ is the mutual information between the discrete variables $A$ and $B$, ${H(p) = -p \log(p) - (1-p) \log(1-p)}$, and \todo{$X^*_i$ is the discretized version of the $i$th sample of $X$}.
