...
The calculation of $h(u,v)$ for all $u \leq v$ has a $\bigo \paren{ n_c \, {L}^{n_s} \cdot N^2 + {L}^{n_p} \cdot N^2 }$ runtime, where $n_c$ and $n_p$ are the numbers of child and parent variables respectively, $L$ is the largest cardinality of variables in $X$'s Markov blanket, and $n_s = \text{max}^{n_c}_{j=1}  J^{(j)}_{S}$.
...
The runtime of Algorithm~\ref{alg:disc_one} is also $\bigo \paren{ n_c \, {L}^{n_s} \cdot N^2 + {L}^{n_p} \cdot N^2 }$, because the runtime of the dynamic programming procedure is less than the runtime for computing $h(u,v)$.
