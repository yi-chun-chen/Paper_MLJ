...
In most cases, we assume that all random variables in Bayesian networks are discrete, since many Bayesian network learning algorithms are unable to deal with continuous variables efficiently.
However, many applications require the use of continuous variables, such as position and velocity in dynamic systems \mykel{cite Kochenderfer JGCD article on encounter modeling}.
...
\mykel{don't like low-cardinality. Maybe 'intervals' or something instead}.
...
\mykel{$\bigstar$ This all needs to be explained better. Too many variables are being defined at once.}
...
The Bayesian network in Figure \ref{fig:wine_graph_1} is obtained by equal-width discretizing each continuous variable into 3 intervals and running K2 1000 times. Table \ref{table:wine_disc_table_1} shows a comparison between the proposed method and the MDL principle discretization method.
...
\todo{Note that the proposed method with the approximation has more intervals after discretization, and it has a slightly better result in terms of likelihood. However, the improvement is marginally small, which indicates that the proposed method without the approximation captures good enough discretization result even if the numbers of intervals after discretization are smaller.}
...
\begin{figure}[ht]
  \centering
    \begin{tabular}{cc}
      \input{graph_wine_1}
    \end{tabular}
  \caption{\tim{Structure from prediscretized K2 structure learning to compare the methods}}
...
\begin{table}
  ...
  \todo{MDL has bins, heuristic is slightly worse in logl but still good. MDL keeps all of its CPDs small, no neighboring vars both have more than 1 interval.}
  \tim{Should we scale the logl? What is the best way to do that?}
  }
...
\begin{figure}[ht]
  ...
  \todo{Note unchanged discretization edge.}
  }
...
\begin{figure}[ht]
  ...
  % \end{tabular}
  \caption{\todo{Structure from prediscretized K2 structure learning to compare the methods for Housing dataset}}
...
\begin{table}
  ...
  \caption{Discretization result of Housing dataset based on the graph Figure~\ref{fig:housing_graph_1} \tim{Fig. vs. Figure}}
...
\begin{figure}[ht]
  ...
  \todo{Mention that there are high-cardinality vars.}
