...
One successful example of parametric, continuous distributions in Bayesian networks is the Gaussian graphical model \citep{Weiss_2011}. \yichun{Is the comma between parametric and continuous necessay?}
...
\yichun{I comment out all computational cost issues}
...
\yichun{A common discretization method for Bayesian networks is to split continuous variables with uniform width or to decide discretization edges based on expert knowledge. A more principled method is the minimum description length (MDL) principle discretization \citep{Friedman_1996}.}
...
The optimal discretization policy of a single continuous variable under MDL can be found using dynamic programming \yichun{with cubic runtime of number of data instances.}
...
Results in this work suggest that the MDL method suffers from low \yichun{sensitivity} to discretization edge locations and returns too few discretization intervals for continuous variables.
...
MODL \citep{Boulle_2006} is a Bayesian method for discretizing a continuous feature according to a class variable. \yichun{
It selects the model with maximum posterior probability given the data. 
Along with Bayes' rule, this is equivalent to maximize ${P(\textit{Model}) \cdot P(\textit{Data} \mid \textit{Model})}$, where ${P(\textit{Data} \mid \textit{Model})}$ usually increases with the number of discretization intervals and $P(\textit{Model})$ is chosen to decrease with the number of discretization intervals. 
...
The four principles,\yichun{ which can be considered an extension of the priors in MODL and \cite{Lustgarten_2011} to Bayesian networks}, are:
...
The procedure used to minimize the objective function involves \yichun{dynamical programming}.
...
This procedure is given in Algorithm~\ref{alg:structure_learn}, and takes five inputs: $D$, a dataset of samples from the joint distribution; $\bm{\cX}$, the set of all continuous variables; \todo{$\texttt{order}$}, a permutation of the variables in $D$; $\hat{n}_\text{parent}$, an upper bound on the number of parents per node; and $\maxpasses$, an upper bound on the number of complete passes.
...
\begin{figure}[H]
  ...
  \caption{
    ...
    The variable $H$ possesses no \todo{overly} repeated values, thus producing a cleaner discretization policy.
  }
