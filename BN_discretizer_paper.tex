...
It is common to discretize all continuous variables before learning a Bayesian network structure to avoid having to consider variable interactions, as the interations and dependencies between variables in Bayesian networks introduce complexity.
...
A common discretization method for continuous variables in Bayesian networks is the minimum description length (MDL) principle discretization method \citep{Friedman_1996}.
The MDL principle \cite{MDL_1978} states that the best model for a dataset is the one that minimizes the amount of information needed to describe it \citep{Grunwald_2009}.
...
The procedure used to minimize the objective function involves \todo{dynamical programming}.
    ...
    \For {$i \leftarrow 1$ to $N$}
      ...
      \While {OKToProceed \textbf{and} $\|\Pa_{X_i}\| < \hat{n}_\text{parent}$}
        ...
        \If {$P_\text{new} > P_\text{old}$}
          ...
          \State $\Lambda \leftarrow$ \textsc{DiscretizeAll}({$D$, $G$, $\bm{\cX '}$, $\maxpasses$}): \quad (Algorithm~\ref{alg:disc_two})
