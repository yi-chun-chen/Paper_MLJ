...
Bayesian networks \citep{Pearl_1988, PGM_2009}, which first emerged from artificial intelligence research, are an increasingly popular method for modeling uncertainty and causality, with applications ranging from decision-making systems \citep{kochenderfer2012next} to medical diagnosis \citep{Lustgarten_2011}.
...
The first is to model the conditional probability density of each continuous variable using specific families of parametric distributions, and then to redesign Bayesian network learning and inference algorithms based on the parameterization.
...
For other parametric distributions and non-linear functions \citep{Ihler_2009}, however, these redesigned algorithms are computationally expensive, and the Gaussian assumption on the underlying data does not always hold.
...
Inference algorithms on nonparametric models are often inefficient due to the large number of parameters they must refer to.
...
These methods search for the best discretization policy for a continuous attribute by considering its interation with a class variable.
Today it is common practice to discretize all continuous variables before learning a Bayesian network structure to avoid having to consider variable interactions, as interations and dependencies between variables in Bayesian networks are more complicated.
...
The predominant discretization method for continuous variables in Bayesian networks is the minimum description length (MDL) principle discretization method \citep{Friedman_1996}.
The MDL principle was first proposed by \cite{MDL_1978}, and states that the best model for a dataset is the one that minimizes the amount of information needed to describe it \citep{Grunwald_2009}.
...
The optimal discretization policy of a single continuous variable under MDL is found using dynamic programming in runtime $\bigo\paren{n^3 + q \cdot n^2}$, where $n$ is the number of data samples and $q$ is a constant with exponential dependency on the cardinalities of the variables in the Bayesian network.
For Bayesian networks with multiple continuous variables, MDL discretization is iteratively applied to each continuous variable.
...
Results in this work suggest that the MDL method suffers from low sensitivity to discretization edge locations and returns low numbers of discretization intervals for continuous variables.
This is caused by MDL's use of mutual information to rate discretization edges.
Mutual information, which is composed of empirical probabilities computed using event count ratios, varies less severely with the positions of discretization edges than the method proposed below.

MODL \citep{Boulle_2006} is a Bayesian method for discretizing a continuous feature according to a class variable.
...
\cite{Lustgarten_2011} suggest several formulations for the prior term $P(\textit{Model})$, and the asymptotical equivalence between MDL and MODL on the single-variable, single-class problem was examined by \citet{VL_2000}.
...
When discussing the discretization of a particular continuous variable $\cX$, let $P_i$ be the $i$th parent of $\cX$, let $C_i$ be the $i$th child of $\cX$, and let $\bm{S}_i$ be the set of spouses of $\cX$ corresponding to the $i$th child.
...
\tim{Is this correct: Note that ${\gamma_i = \sum_l n^{(j)}_{i,\ell}}$ for all $j$.}
It follows that
...
\tim{Why not Eq~\ref{eq:likelihood_two}?}
...
\tim{Stress that this is an original contribution}
\tim{Maybe stress the runtime complexity?}
...
Note that because the objective function is cumulative over intervals, if a partition of $\cX$ into $k$ intervals by $\Lambda = \anglebrackets{\gamma_1, \gamma_2, \ldots, \gamma_k}$ is an optimal discretization policy, then any subinterval is optimal for the corresponding subproblem.
...
\tim{If it only fails to converge with structure learning, perhaps we should take out the max upper bound in Alg~\ref{alg:disc_two}.}
...
In practice, one maximizes the log-likelihood, also known as the Bayesian score~\citep{cooper1992bayesian}:
...
\noindent
where $\mathrm{\Gamma}$ is the gamma function, $\alpha_{ij}^{(k)}$ is a Dirichlet prior count and $\beta_{ij}^{(k)}$ is the observed sample count for the $k$th instantiation of $X_i$ and the $j$th instantiation of $\Pa_{X_i}$.
...
The discretization methods can be combined with the K2 structure learning algorithm \citep{K2} to simultaneously perform Bayesian structure learning and discretization of continuous variables.
...
The upper bound on the number of parents is common practice, and is done to prevent computing conditional distribtions with excessively large parameter sets. \tim{cite?}
...
\begin{algorithm}
  ...
  \Function{Learn\_DVBN}{$D$, $\bm{\cX}$, $\texttt{order}$, $\hat{n}_\text{parent}$, $\maxpasses$}
    ...
    \For {$i \leftarrow 1$ to $N$}
      \State $P_\text{old} \leftarrow g(X_i,\Pa_{X_i})$ per Equation~\ref{eq:B_Score_one}
      ...
      \While {OKToProceed \textbf{and} $\|\Pa_{X_i}\| < \hat{n}_\text{parent}$}
        ...
        \If {$P_\text{new} > P_\text{old}$}
          ...
          \State $\Lambda \leftarrow$ \textsc{DiscretizeAll}({$D$, $G$, $\bm{\cX '}$, $\maxpasses$}): see Algorithm~\ref{alg:disc_two}
...
Equation \ref{eqn:MDL} can be minimized using dynamical programming with runtime $\bigo \paren{ n^3 + \paren{ n_c \cdot {L}^{n_s}  + {L}^{n_p}} \cdot n^2 }$.
All variables are defined in Section \ref{subsec:algo}.
Therefore, the runtime of the proposed method in this paper is shorter than MDL discretization method by $\bigo \paren{n^3}$.
\tim{It seems weird having this result being explained in the experiments section.}
...
To quantatively discuss the sensitivity of each method to the number of discretization intervals, consider the Bayesian network in Figure~\ref{fig:exp_discuss}, where $\cX$ is continuous and $P_1$ and $P_2$ are discrete.
...
The value of the edge position term for MDL is primarly determined by the mutual information, which varies less severely than the corresponding terms for the Bayesian method.
The MDL term uses emperical probability distributions, based off of ratios of counts, whereas the Bayesian method uses factorial terms, and thus vary less severely.
...
In addition, this paper shows how to incorporate existing methods into the structure learning process to discretize all continuous variables and simultaneously learn Bayesian network structures.
The proposed method was incorporated and its superior performance was empirically demonstrated.
All software is publically available to support discretization research, \todo{and can be found at.}
...
Many edges appears in both discretization results, and are bolded in Table~\ref{table:auto_disc_table_3}.

\begin{figure}[ht]
  \centering
  \scalebox{0.8}{\input{graph_autocar_3}}
  \caption{A Bayesian network structure for the Auto MPG dataset that leads to comparable discretization policies from the MDL and Bayesian methods.}
...
\begin{table}[h]
  ...
  \caption{
    ...
    The methods have same discretization result on variable $A$, and the log-likelihood for the MDL method only just slightly worse than that of the Bayesian method.
  }
