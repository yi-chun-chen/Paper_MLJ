...
% To quantatively discuss the sensitivity of the three methods, consider the Bayesian network in Figure \ref{fig:exp_discuss}, where $X$ is continuous-valued and $P_1$ and $P_2$ is discrete-valued. Let $P = \{  P_1, P_2 \}$. Assume $D_X = \{1.0,2.0,3.0, \ldots N \}$. Therefore, the corresponding objective functions to discretize $X$ in the three methods are:
% \begin{equation}
% \begin{aligned}
% \text{Obj}_{\text{MDL}} & = Z_1 \cdot k + \log(N) \cdot \sum^k_{i=1} \sum^{J_P}_{j=1} I(a^*_i, (p_1,p_2)_j) \\
% \text{Obj}_{\text{Bayesian}} &= Z_2 \cdot k + \log {{n_i + J_P - 1}\choose{J_P -1}} + \sum^k_{i=1} \log \left(   {{n_i !}\over{n^{(P)}_{i,1}! n^{(P)}_{i,2}! \cdots n^{(P)}_{i,J_P}!    }} \right) \\
% \text{Obj}_{\text{Heuristic}} &= Z_2 \cdot k + \log {{n_i + J_{P_1} - 1}\choose{J_{P_1} -1}} + \log {{n_i + J_{P_2} - 1}\choose{J_{P_2} -1}} +\\
% & \sum^k_{i=1}  \left[ \log \left(   {{n_i !}\over{n^{(P_1)}_{i,1}! n^{(P_1)}_{i,2}! \cdots n^{(P_1)}_{i,J_{P_1}}    }} \right)  + \log \left(   {{n_i !}\over{n^{(P_2)}_{i,1}! n^{(P_2)}_{i,2} \cdots n^{(P_2)}_{i,J_{P_2}} }} \right) \right]
% \end{aligned}
% \end{equation}
