...
\begin{algorithm}
  ...
  \label{alg:disc_two}
  \begin{algorithmic}[5]
  \Function{DiscretizeAll}{$D$, $G$, $C$, $u_{cycle}$}
    ...
    \While {$\Lambda$ is not converged  \textbf{and} $n_{\text{{cycle}}} \leq u_{cycle}$}
      \State {$n_{\text{{cycle}}}  \leftarrow n_{\text{{cycle}}} + 1$}
      \For {$X_j$ in $C$}
        \State $D' \leftarrow D^*$
        \State $D'_{X_j}$ is replaced by $D_{X_j}$
        \State {$\Lambda^{(j)} \leftarrow $ \textsc{Discretize}\text{One}($X_j$, $D'$, $G$)}
        \State {$D^*_{X_j}  \leftarrow$ $\Lambda^{(j)}(D_{X_j})$}
      \EndFor
    \EndWhile
...
\begin{algorithm}
  ...
  \Function{Learn\_DBN}{$D$, $C$, $order$, $u_{parent}$, $C$, $u_{cycle}$}
    ...
    \For {$i \leftarrow 1$ to $n$}
      ...
      \While {OKToProceed \textbf{and} $\|\Pi_{X_i}\| < u_{parent}$}
